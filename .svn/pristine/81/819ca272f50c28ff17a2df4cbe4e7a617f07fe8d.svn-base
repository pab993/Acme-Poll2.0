...
public class MessageActorController extends AbstractController {
	...
	public ModelAndView spam(@RequestParam final int messageId) {
		...
		try {
			Assert.isTrue(message.getFolder().getActor().getId() == this.actorService.findByPrincipal().getId());
			final Folder destinyFolder = this.folderService.findActorAndFolder(this.actorService.findByPrincipal().getId(), "spambox");
	...
	protected ModelAndView createEditModelAndView2(final Message message) {
		ModelAndView result;
	...
	protected ModelAndView createEditModelAndView2(final Message msg, final String text) {
		ModelAndView result;
