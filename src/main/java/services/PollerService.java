...
public class PollerService {
	...
	public Poller reconstruct(PollerForm pollerForm, BindingResult binding) {
		...
		comprobarPostalAddress(pollerForm.getPostalAddress(), binding);
	...
	private boolean comprobarPostalAddress(final String postalAddress, final BindingResult binding) {
		...
		if (!result)
			if (postalAddress.matches("^[0-9]{5}$"))
				...
				codigos[0] = "poller.postalAddress.error";
				error = new FieldError("pollerForm", "postalAddress", postalAddress, false, codigos, null, "");
